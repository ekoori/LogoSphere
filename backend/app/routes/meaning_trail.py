...
def get_exchange(exchange_id, user_id=None):
    if request.method == 'OPTIONS':
        return app.make_default_options_response(), 200
    try:
        tx_dict, is_initiator = MeaningTrail.get_by_id(exchange_id, user_id)
        if tx_dict is None:
            return jsonify({'message': 'Exchange not found or not authorized'}), 404
        return jsonify({'exchange': tx_dict, 'is_initiator': is_initiator}), 200
    except Exception as e:
...
def update_xc_status(exchange_id, user_id=None):
    ...
    try:
        ...
        tx_dict, _ = MeaningTrail.get_by_id(exchange_id, user_id)
        if tx_dict is None:
            return jsonify({'message': 'Exchange not found or not authorized'}), 404
